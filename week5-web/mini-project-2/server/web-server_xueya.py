...
def get_client_rate(client_id):
    ...
    client_rate = client_dict[client_id]['rate']
    return client_rate
# -- TODO END: Part 1
...
def update_client_rates(client_id, rate):
    ...
    if client_id in client_info:
        client_info[client_id]['rate'] = rate
        return "updated"
    else:
        client_info.append(client_id,{'rate':rate})
        return "inserted"
    # re-write the file
    client_info = pd.DataFrame(client_info)
    client_info.to_json("client_rate.json")
    pass
# -- TODO END: Part 4
